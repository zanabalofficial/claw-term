...
export class TaskPlanner extends EventEmitter {
    ...
    createTask(plan, taskData, parentId) {
        const task = {
            id: uuidv4(),
    ...
    validatePlan(planId, availableTools) {
    ...
    hasCircularDependency(plan, taskId, visited = new Set()) {
    ...
    // Update task status
    updateTaskStatus(planId, taskId, status, result, error) {
        const plan = this.plans.get(planId);
        if (!plan)
            return;
        const task = plan.tasks.get(taskId);
