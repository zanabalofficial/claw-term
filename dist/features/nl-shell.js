...
Respond in JSON format:
{
  ...
  "safeAlternative": "safer version if dangerous"
    ...
    async executeWithSafety(command, confirmed = false) {
    ...
    async interactiveExecute(naturalLanguage) {
    ...
    async suggestCommands(currentDirectory, recentFiles) {
        const provider = createProvider(this.provider, {
            apiKey: this.apiKey,
            model: 'gpt-4',
            temperature: 0.7,
        ...
        let response = '';
        await provider.streamResponse([{ role: 'user', content: prompt }], {
            onToken: (token) => { response += token; },
            onToolCall: () => { },
            onComplete: () => { },
            onError: () => { },
