...
5. Use exec and process tools for operations`,
    ...
    async executeComplexTask(taskDescription) {
        ...
        const plan = await this.runAgent(coordinator, `Break down this task into subtasks and assign them to the appropriate agents: ${taskDescription}\n\n` +
        ...
        while (completedTasks.size < subtasks.length) {
            const readyTasks = subtasks.filter(t => t.status === 'pending' &&
        ...
        const allResults = Array.from(agentOutputs.entries())
            .map(([id, result]) => `=== ${id} ===\n${result}`)
            .join('\n\n');
        const finalResult = await this.runAgent(coordinator, `Synthesize these results into a coherent final answer:\n\n${allResults}`);
