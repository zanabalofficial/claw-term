...
`;
    ...
    async addDocuments(dbName, documents, model) {
        const config = this.configs.get(dbName);
        if (!config)
            throw new Error(`Vector DB ${dbName} not found`);
        // Generate embeddings for all documents
        const embeddings = await Promise.all(documents.map(doc => this.embed(doc.content, model)));
        switch (config.type) {
            case 'chromadb':
    ...
    async addToChroma(config, documents, embeddings) {
        const script = `
import chromadb
import json
...
`;
    ...
    async searchChroma(config, queryEmbedding, options) {
        const script = `
import chromadb
import json
...
`;
    ...
    async hybridSearch(dbName, query, model, options = {}) {
    ...
    async keywordSearch(dbName, query, options) {
