...
export class DatabaseTools extends EventEmitter {
    ...
    // Execute SQL query
    async query(connectionName, sql) {
    ...
    async queryPostgres(conn, sql, startTime) {
        const env = {
    ...
    async queryMySQL(conn, sql, startTime) {
        const args = [
            '-h', conn.host || 'localhost',
    ...
    async querySQLite(conn, sql, startTime) {
    ...
    async queryMongo(connectionName, collection, operation, args) {
        const conn = this.connections.get(connectionName);
        if (!conn)
            throw new Error(`Connection ${connectionName} not found`);
...
`;
    ...
    // Redis operations
    async redisCommand(connectionName, command, ...args) {
        const conn = this.connections.get(connectionName);
        if (!conn)
            throw new Error(`Connection ${connectionName} not found`);
        if (conn.type !== 'redis')
            throw new Error('Not a Redis connection');
        const redisCli = [
            '-h', conn.host || 'localhost',
    ...
    async listTables(connectionName) {
        ...
        switch (conn.type) {
            case 'postgresql':
                const pgResult = await this.query(connectionName, "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'");
                return pgResult.rows.map(r => r.table_name);
            case 'mysql':
                const mysqlResult = await this.query(connectionName, 'SHOW TABLES');
                return mysqlResult.rows.map(r => Object.values(r)[0]);
            case 'sqlite':
                const sqliteResult = await this.query(connectionName, "SELECT name FROM sqlite_master WHERE type = 'table'");
    ...
    // Get table schema
    async getSchema(connectionName, table) {
        const conn = this.connections.get(connectionName);
        if (!conn)
            throw new Error(`Connection ${connectionName} not found`);
        let sql;
