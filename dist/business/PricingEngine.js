...
export class PricingEngine extends EventEmitter {
    ...
    calculate(agentId, usage, outcome) {
    ...
    calculateOutcome(config, outcome) {
    ...
    calculateHybrid(config, usage, outcome) {
        const base = config.baseFee || 500;
        // Usage component (30% weight)
        const usageComponent = this.calculateUsage({ ...config, usageRate: (config.usageRate || 1) * 0.3 }, usage);
        // Outcome component (50% weight after base)
        const outcomeComponent = this.calculateOutcome({ ...config, outcomeRate: (config.outcomeRate || 0.2) * 0.5 }, outcome);
    ...
    applyLimits(amount, config) {
        let result = amount;
        if (config.minFee && result < config.minFee)
            result = config.minFee;
        if (config.maxFee && result > config.maxFee)
    ...
    // Generate invoice
    generateInvoice(agentId, period, usage, outcome) {
