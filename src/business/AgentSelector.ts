...
export interface SelectionCriteria {
  pain: number;        // P: 1-5 (pain intensity)
  budget: number;      // B: 1-5 (budget availability)
  measurability: number; // M: 1-5 (how clear is ROI)
  automatable: number; // A: 1-5 (depth of possible automation)
  data: number;        // D: 1-5 (data accessibility)
  regulation: number;  // R: 1-5 (low drag = high score)
  wedge: number;       // W: 1-5 (distribution advantage)
}
...
export class AgentSelector extends EventEmitter {
  private opportunities: Map<string, AgentOpportunity> = new Map();
  ...
  private addOpportunity(opp: AgentOpportunity): void {
  ...
  score(agentId: string): ScoringResult {
    ...
    let interpretation: ScoringResult['interpretation'];
  ...
  getRecommendations(tier?: 1 | 2 | 3, limit: number = 5): AgentOpportunity[] {
  ...
  }): { overall: 'likely_good' | 'likely_weak'; checks: ValidationCheck[] } {
