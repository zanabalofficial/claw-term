...
export class ComputerUse {
  ...
  constructor() {
    this.platform = process.platform as 'linux' | 'darwin' | 'win32';
  ...
  // Take screenshot
  async screenshot(region?: ScreenRegion): Promise<Screenshot> {
...
"`);
  ...
  async screenshotWithOCR(region?: ScreenRegion): Promise<{
  ...
  // Close application
  async closeApplication(appName: string): Promise<void> {
    let command: string;
